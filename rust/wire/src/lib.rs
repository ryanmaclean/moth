...
//!
//! `scan_for_byte` and `scan_for_pair` are the workhorses for SSE
//! framing (`\n\n`), NDJSON splitting (`\n`), template scanning (`{{`),
//! and shell-expression scanning (`` ` ``) — all of which we'll build on top.
...
mod neon {
    ...
    /// pair of input bytes collapses to one output nibble. Returns the byte
    /// index of the first 0xFF lane, or `None` if all lanes are zero.
    ///
    /// This is the canonical NEON "movemask" replacement: `vshrn_n_u16(v, 4)`
    /// is one cycle on modern Arm cores and saves the SSE2-style ratio of
    /// instructions to extract the result.
...
mod tests {
    ...
    fn pair_overlap_does_not_double_count() {
        // Three consecutive `{` should report position 0, not 1.
